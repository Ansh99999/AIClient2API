...
const translations = {
    'zh-CN': {
        ...
        'upload.importBackup': '导入备份',
        'upload.importBackup.hint': '导入备份包（ZIP），配置会还原到各自的位置',
        'upload.importBackup.title': '导入备份',
        'upload.importBackup.resultTitle': '导入结果',
        'upload.importBackup.archive': '备份包:',
        'upload.importBackup.layout': '识别到的结构:',
        'upload.importBackup.layout.configs-dir': '包内含 configs/ 目录',
        'upload.importBackup.layout.configs-relative': '包内条目相对 configs/',
        'upload.importBackup.conflict': '同名文件:',
        'upload.importBackup.conflict.overwrite': '覆盖现有文件',
        'upload.importBackup.conflict.skip': '保留现有文件',
        'upload.importBackup.conflict.rename': '另存为新文件',
        'upload.importBackup.includeSensitive': '同时恢复管理密码 (pwd) 和登录会话 (token-store.json)。导入别人机器上的备份时勾选它可能导致你登不进来。',
        'upload.importBackup.note': '导入前会自动把当前 configs/ 打一份快照放到 configs/.backups/，最多保留 5 份。恢复完成后会自动重载配置。',
        'upload.importBackup.doneNote': '带 → 的行是实际落点。configs/config.json 与 configs/provider_pools.json 已重载；如果账号池仍未生效，可以重启服务。',
        'upload.importBackup.confirm': '开始导入',
        'upload.importBackup.analyzing': '正在解析备份包...',
        'upload.importBackup.analyzeFailed': '解析备份包失败',
        'upload.importBackup.processing': '正在导入备份...',
        'upload.importBackup.success': '成功恢复 {count} 个文件',
        'upload.importBackup.failed': '导入备份失败',
        'upload.importBackup.nothingRestored': '没有文件被恢复',
        'upload.importBackup.willRestore': '恢复 {count} 个',
        'upload.importBackup.willSkip': '跳过 {count} 个',
        'upload.importBackup.didFail': '失败 {count} 个',
        'upload.importBackup.snapshot': '导入前快照:',
        'upload.importBackup.reason.unsafe_or_ignored': '路径不安全或无需恢复',
        'upload.importBackup.reason.outside_configs': '不在 configs/ 目录内',
        'upload.importBackup.reason.not_a_backup': '这个压缩包不像一份配置备份',
        'upload.importBackup.reason.app_file': '程序本体的文件，不是配置',
        'upload.importBackup.reason.excluded_dir': '临时目录/历史快照，不恢复',
        'upload.importBackup.reason.sensitive': '敏感文件，未勾选恢复',
        'upload.importBackup.reason.duplicate_target': '与前一个条目落点相同',
        'upload.importBackup.reason.exists': '已存在，按设置保留现有文件',
    ...
    'en-US': {
        ...
        'upload.importBackup': 'Import Backup',
        'upload.importBackup.hint': 'Import a backup archive (ZIP); every config is restored to its own place',
        'upload.importBackup.title': 'Import Backup',
        'upload.importBackup.resultTitle': 'Import Result',
        'upload.importBackup.archive': 'Archive:',
        'upload.importBackup.layout': 'Detected layout:',
        'upload.importBackup.layout.configs-dir': 'archive contains a configs/ directory',
        'upload.importBackup.layout.configs-relative': 'entries are relative to configs/',
        'upload.importBackup.conflict': 'Existing files:',
        'upload.importBackup.conflict.overwrite': 'Overwrite them',
        'upload.importBackup.conflict.skip': 'Keep them',
        'upload.importBackup.conflict.rename': 'Save alongside them',
        'upload.importBackup.includeSensitive': 'Also restore the admin password (pwd) and login sessions (token-store.json). Ticking this for a backup from another machine can lock you out.',
        'upload.importBackup.note': 'A snapshot of the current configs/ is taken into configs/.backups/ before anything is written; the last 5 are kept. The configuration is reloaded once the restore finishes.',
        'upload.importBackup.doneNote': 'Rows with → show where each file actually landed. configs/config.json and configs/provider_pools.json have been reloaded; restart the service if the provider pool still looks stale.',
        'upload.importBackup.confirm': 'Start Import',
        'upload.importBackup.analyzing': 'Reading the backup archive...',
        'upload.importBackup.analyzeFailed': 'Failed to read the backup archive',
        'upload.importBackup.processing': 'Importing the backup...',
        'upload.importBackup.success': 'Restored {count} files',
        'upload.importBackup.failed': 'Failed to import the backup',
        'upload.importBackup.nothingRestored': 'No files were restored',
        'upload.importBackup.willRestore': '{count} restored',
        'upload.importBackup.willSkip': '{count} skipped',
        'upload.importBackup.didFail': '{count} failed',
        'upload.importBackup.snapshot': 'Snapshot taken:',
        'upload.importBackup.reason.unsafe_or_ignored': 'unsafe path or nothing to restore',
        'upload.importBackup.reason.outside_configs': 'not inside configs/',
        'upload.importBackup.reason.not_a_backup': 'this archive does not look like a configs backup',
        'upload.importBackup.reason.app_file': 'part of the application, not a config',
        'upload.importBackup.reason.excluded_dir': 'scratch directory / older snapshot',
        'upload.importBackup.reason.sensitive': 'sensitive file, not opted in',
        'upload.importBackup.reason.duplicate_target': 'another entry already claims this path',
        'upload.importBackup.reason.exists': 'already exists, kept as-is',
